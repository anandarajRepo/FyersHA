...
echo "$(date): Attempting to stop HA scalping process..." >> /var/log/ha.log
...
# Log completion
echo "$(date): Stop script completed" >> /var/log/ha.log

# Optional: Check if process is still running
if pgrep -f "main.py run" > /dev/null; then
    echo "$(date): WARNING - Process still running!" >> /var/log/ha.log
else
    echo "$(date): Process successfully stopped" >> /var/log/ha.log
fi